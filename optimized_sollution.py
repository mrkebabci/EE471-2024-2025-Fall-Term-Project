import json
import math
import numpy as np
from scipy import sparse


def create_bus_admittance_matrix(filename):
    with open(filename) as f:
        data = json.load(f)

    # init
    y_bus_size = len(data["bus_data"])
    bus_name_to_index = {bus_name: index for index, bus_name in enumerate(data["bus_data"])}
    y_bus = np.zeros((y_bus_size, y_bus_size), dtype=complex)

    def compute_admittances(line_info):
        R_pu = float(line_info["R_pu"])
        X_pu = float(line_info["X_pu"])
        B_pu = float(line_info["B_pu"])

        series_admittance = 1 / complex(R_pu, X_pu)
        shunt_admittance = complex(0, B_pu) / 2

        #Calculate admittance entries based on line type
        line_type = line_info["type"]
        if line_type == "0":  # Transmission Line
            return series_admittance + shunt_admittance, -series_admittance, -series_admittance, series_admittance + shunt_admittance
        elif line_type == "1":  # Standard Transformer
            return series_admittance, -series_admittance, -series_admittance, series_admittance
        elif line_type == "2":  # Tap Changer Transformer
            tap = float(line_info["tap"])
            if tap != 0:
                return series_admittance / tap ** 2, -series_admittance / tap, -series_admittance / tap, series_admittance
            else:
                return 0, 0, 0, 0
        elif line_type == "3":  # Phase Shifter Transformer
            return 0, 0, 0, 0
        elif line_type == "4":  # Phase Shifter Transformer
            phase_shift_rad = np.deg2rad(float(line_info["phase_shift_degree"]))
            phase_shift_complex = complex(math.cos(phase_shift_rad), math.sin(phase_shift_rad))
            return (series_admittance,
                    -series_admittance / np.conjugate(phase_shift_complex),
                    -series_admittance / phase_shift_complex,
                    series_admittance)

    #Update y_bus entries
    for k, line_info in data["branch_data"].items():
        from_bus_name = line_info["from_bus"]
        to_bus_name = line_info["to_bus"]

        try:
            from_index = bus_name_to_index[from_bus_name]
            to_index = bus_name_to_index[to_bus_name]

            y_ii, y_ij, y_ji, y_jj = compute_admittances(line_info)

            y_bus[from_index, from_index] += y_ii
            y_bus[from_index, to_index] += y_ij
            y_bus[to_index, from_index] += y_ji
            y_bus[to_index, to_index] += y_jj

        except KeyError:
            continue
    print(y_bus)
    #Add bus shunt elements from bus data
    for k, bus_info in data["bus_data"].items():
        bus_name = k
        bus_index = bus_name_to_index[bus_name]

        G_pu = float(bus_info["G_pu"])
        B_pu = float(bus_info["B_pu"])

        if (G_pu != 0) or (B_pu != 0):
            y_bus[bus_index, bus_index] += complex(G_pu,B_pu)

    print(y_bus)
    return sparse.csr_matrix(y_bus)

filename = "ieee14bus.json"
ans = create_bus_admittance_matrix(filename)
print(ans)